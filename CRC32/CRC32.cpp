...
std::string CRC32::ResultToHex( unsigned long Result )
{
	char buf[10] = { 0 };
...
std::string CRC32::CRCGet( unsigned char* buffer, unsigned long size, bool release)
{
	unsigned long CRCResult;
	CRCInit(CRCResult);
	CRCUpdate(buffer, size, CRCResult);
	CRCFinal(CRCResult);
	if(release) {
		delete[] buffer;
	}
